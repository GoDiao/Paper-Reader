...
def convert_markdown_table_to_latex(text):
    ...
    if len(split_column) > 1 and re.match(r'^\|\s*:?-+:?\s*(\|\s*:?-+:?\s*)+\|\s*$', split_column[1]):
        # Remove the delimiter line
        split_column.pop(1)
        has_header = True
    def _escape_text_segment(segment):
        if not segment:
            return ""
        segment = segment.replace("\\", "\\textbackslash{}")
        segment = segment.replace("&", "\\&")
        segment = segment.replace("{", "\\{").replace("}", "\\}")
        segment = segment.replace("%", "\\%")
        segment = segment.replace("#", "\\#")
        segment = segment.replace("_", "\\_")
        segment = segment.replace("^", "\\textasciicircum{}")
        segment = segment.replace("~", "\\textasciitilde{}")
        segment = segment.replace("$", "\\$")
        return segment

    def _cell_to_latex(cell, is_header):
        cell = cell.strip()
        if not cell:
            return ""
        if cell.count("$") % 2 != 0:
        ...
        for part in parts:
            ...
            if part.startswith("$") and part.endswith("$") and len(part) >= 2:
                ...
                if math_content:
                    rendered.append(math_content)
            else:
                text_seg = _escape_text_segment(part)
                if text_seg:
                    if is_header:
                        rendered.append(f"\\textsf{{\\textbf{{{text_seg}}}}}")
                    else:
                        rendered.append(f"\\textsf{{{text_seg}}}")
    ...
    for i, row in enumerate(split_column):
        ...
        for j, cell in enumerate(modified_content):
            cell_text = _cell_to_latex(cell, i == 0 and has_header)
    ...
    first_row_cells = re.findall(r'(?<=\|).*?(?=\|)', split_column[0]) if split_column else []
    count_column = max(1, len(first_row_cells))
...
def convert_markdown_table_to_notion_blocks(text):
    def _split_row(line):
        cells = []
        current = []
        in_math = False
        ...
        while i < len(line):
            ...
            if ch == "`" and prev != "\\":
                ...
                i += 1
                continue
            if ch == "$" and prev != "\\" and not in_code:
                in_math = not in_math
                current.append(ch)
                i += 1
                continue
            if ch == "|" and prev != "\\" and not in_math and not in_code:
            ...
            i += 1
        cells.append("".join(current))
        if line.lstrip().startswith("|") and cells and cells[0].strip() == "":
            cells = cells[1:]
        if line.rstrip().endswith("|") and cells and cells[-1].strip() == "":
            cells = cells[:-1]
    ...
    col_count = max(1, max(len(r) for r in rows))
    normalized_rows = []
    for r in rows:
    ...
    blocks = []
    start_index = 0
    while start_index < len(normalized_rows):
        if has_header and start_index > 0:
            rows_chunk = normalized_rows[start_index:start_index + max_rows_per_table - 1]
            chunk = [normalized_rows[0]] + rows_chunk
            start_index += max_rows_per_table - 1
        else:
            chunk = normalized_rows[start_index:start_index + max_rows_per_table]
            start_index += max_rows_per_table
        table_block = {
            "object": "block",
            "type": "table",
            "table": {
        ...
        for row in chunk:
            cells = []
            for cell in row:
                rich = process_inline_formatting(cell)
                cells.append(rich if rich else [])
...
def parse_markdown_to_notion_blocks(markdown):
    ...
    def _list_depth(indent_spaces):
        if indent_spaces <= 0:
            return 0
        if indent_spaces % 4 == 0:
            return indent_spaces // 4
        return indent_spaces // 2
    ...
    for line in lines:
        ...
        is_table_delimiter = re.match(r'^\|\s*:?-+:?\s*(\|\s*:?-+:?\s*)+\|\s*$', line)
        ...
        if is_table_row or is_table_delimiter:
            ...
            if len(current_table) >= 2:
                table_blocks = convert_markdown_table_to_notion_blocks(table_str)
                if table_blocks:
                    ...
                    blocks.append({
                        ...
                        "type": "paragraph",
                        "paragraph": {
                            "rich_text": process_inline_formatting(table_str)
                        }
                    })
            else:
                blocks.append({
        ...
        if list_match:
            ...
            if depth == current_depth:
                ...
                prev = stack[-1][-1] if stack[-1] else None
                target_depth = min(depth, current_depth + 1)
                if isinstance(prev, dict) and target_depth > current_depth:
                    if prev.get('type') == 'numbered_list_item' and 'numbered_list_item' in prev:
                        container = prev['numbered_list_item']
                    elif prev.get('type') == 'bulleted_list_item' and 'bulleted_list_item' in prev:
                        container = prev['bulleted_list_item']
                    else:
                        container = None
                    if container is not None:
                        ...
                        stack.append(container['children'])
                        current_depth += 1
                    else:
                        stack[-1].append(item)
                else:
        ...
        if list_match:
            ...
            if depth == current_depth:
                ...
                target_depth = min(depth, current_depth + 1)
                if isinstance(prev, dict) and target_depth > current_depth:
                    if prev.get('type') == 'bulleted_list_item' and 'bulleted_list_item' in prev:
                        container = prev['bulleted_list_item']
                    elif prev.get('type') == 'numbered_list_item' and 'numbered_list_item' in prev:
                        container = prev['numbered_list_item']
                    else:
                        container = None
                    if container is not None:
...
def create_notion_page_from_md(markdown_text, title, parent_page_id, cover_url='', parent_type='page', properties=None, title_property_name='Name', print_page_info=False, print_database_info=False):
    """
    ...
    if parent_type == 'page':
        ...
        notion.pages.update(created_page["id"], **update_kwargs)
    else:
        if cover_url != "":
            notion.pages.update(
                created_page["id"],
        ...
        if print_database_info:
            pprint.pprint(notion.databases.retrieve(database_id=parent_page_id))
